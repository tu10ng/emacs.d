...
#+end_src
*** TODO auto-insert
need insert different thing for major mode 
#+begin_src emacs-lisp
...
(defun auto-insert ()
  (let ((res (assoc (char-before)
                    auto-insert-alist)))
    (when res
      (insert (cdr res)))))

(define-minor-mode auto-insert-mode
  "Automatically insert char after certain char in specific modes."
  :init-value nil
  :lighter "._a"
  :keymap nil
  (make-variable-buffer-local 'post-self-insert-hook)
  (if auto-insert-mode
      (add-hook 'post-self-insert-hook 'auto-insert)
    (remove-hook 'post-self-insert-hook 'auto-insert)))
