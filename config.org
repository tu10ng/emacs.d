...
;;                 (delete-window))))
(defun tu10ng/kill-current-buffer-delete-window ()
  (interactive)
  (kill-current-buffer)
  (ignore-errors
    (delete-window)))

(global-set-key (kbd "C-x k") #'tu10ng/kill-current-buffer-delete-window)
#+end_src
*** restore session
#+BEGIN_SRC emacs-lisp
...
;; fix (error "Specified window is not displaying the current buffer") 
;; this error showed up after emacs 31
...
fragment part of ENTRY.path."
  (with-current-buffer (get-buffer-create "*devdocs*")
    (display-buffer (get-buffer-create "*devdocs*"))
